...
const Lightening = artifacts.require('./ERC20Lightening.sol')
...
contract('ERC20Lightening', accounts => {
  let lightening
  let token
  let usdtToken
  let badRecipient
  ...
  before(async () => {
    ...
    if (ERC20_TOKEN) {
      ...
      await token.mint(sender, toBN(tokenDenomination).mul(toBN(10005)).div(toBN(10000)).toString(10))
    }
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const tokenFromContract = await lightening.token()
  ...
  describe('#deposit', () => {
    it('should work', async () => {
      ...
      await token.approve(lightening.address, toBN(tokenDenomination).mul(toBN(10005)).div(toBN(10000)).toString(10))

      let { logs } = await lightening.deposit(commitment, { from: sender })
    ...
    it('should not allow to send ether on deposit', async () => {
      ...
      let error = await lightening.deposit(commitment, { from: sender, value: 1e6 }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      await token.mint(user, toBN(tokenDenomination).mul(toBN(10005)).div(toBN(10000)).toString(10))

      const balanceUserBefore = await token.balanceOf(user)
      await token.approve(lightening.address, toBN(tokenDenomination).mul(toBN(10005)).div(toBN(10000)).toString(10), { from: user })
      // Uncomment to measure gas usage
      // let gas = await lightening.deposit.estimateGas(toBN(deposit.commitment.toString()), { from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await lightening.deposit(toFixedHex(deposit.commitment), { from: user, gasPrice: '0' })

      const balanceUserAfter = await token.balanceOf(user)
      const depositFee = toBN(tokenDenomination).mul(toBN(5)).div(toBN(10000))
      balanceUserAfter.should.be.eq.BN(toBN(balanceUserBefore).sub(toBN(tokenDenomination)).sub(depositFee))
      ...
      const balanceLighteningBefore = await token.balanceOf(lightening.address)
      ...
      const { logs } = await lightening.withdraw(proof, ...args, { value: refund, from: relayer, gasPrice: '0' })

      const balanceLighteningAfter = await token.balanceOf(lightening.address)
      ...
      balanceLighteningAfter.should.be.eq.BN(toBN(balanceLighteningBefore).sub(toBN(tokenDenomination)))
      balanceRelayerAfter.should.be.eq.BN(toBN(balanceRelayerBefore).add(feeBN))
      const withdrawFee = toBN(tokenDenomination).sub(feeBN).mul(toBN(10)).div(toBN(10000))
      balanceRecieverAfter.should.be.eq.BN(toBN(balanceRecieverBefore).add(toBN(tokenDenomination).sub(feeBN).sub(withdrawFee)))
    ...
    it('should reject with wrong refund value', async () => {
      ...
      let { reason } = await lightening.withdraw(proof, ...args, { value: 1, from: relayer, gasPrice: '0' }).should.be.rejected
      reason.should.be.equal('Incorrect refund amount received by the contract')


        ; ({ reason } = await lightening.withdraw(proof, ...args, { value: toBN(refund).mul(toBN(2)), from: relayer, gasPrice: '0' }).should.be.rejected)
      reason.should.be.equal('Incorrect refund amount received by the contract')
    })
    ...
    it.skip('should work with REAL USDT', async () => {
      ...
      await tree.insert(deposit.commitment)
      await usdtToken.transfer(user, toBN(tokenDenomination).mul(toBN(10005)).div(toBN(10000)).toString(10), { from: sender })
      console.log('transfer done')

      const balanceUserBefore = await usdtToken.balanceOf(user)
      console.log('balanceUserBefore', balanceUserBefore.toString())
      await usdtToken.approve(lightening.address, toBN(tokenDenomination).mul(toBN(10005)).div(10000), { from: user })
      console.log('approve done')
      const allowanceUser = await usdtToken.allowance(user, lightening.address)
      ...
      const balanceLighteningBefore = await usdtToken.balanceOf(lightening.address)
      ...
      const ethBalanceRecieverBefore = await web3.eth.getBalance(toFixedHex(recipient, 20))
      let isSpent = await lightening.isSpent(input.nullifierHash.toString(16).padStart(66, '0x00000'))
      isSpent.should.be.equal(false)
      ...
      const balanceLighteningAfter = await usdtToken.balanceOf(lightening.address)
      ...
      logs[0].args.fee.should.be.eq.BN(feeBN)
      isSpent = await lightening.isSpent(input.nullifierHash.toString(16).padStart(66, '0x00000'))
      isSpent.should.be.equal(true)
    })
    it.skip('should work with REAL DAI', async () => {
      ...
      console.log('senderBal', senderBal.toString())
      await tree.insert(deposit.commitment)
      await token.transfer(user, toBN(tokenDenomination).mul(toBN(10005)).div(toBN(10000)).toString(10), { from: sender })
      console.log('transfer done')

      const balanceUserBefore = await token.balanceOf(user)
      console.log('balanceUserBefore', balanceUserBefore.toString())
      await token.approve(lightening.address, toBN(tokenDenomination).mul(toBN(10005)).div(10000), { from: user })
      console.log('approve done')
      await lightening.deposit(toFixedHex(deposit.commitment), { from: user, gasPrice: '0' })
      console.log('deposit done')
