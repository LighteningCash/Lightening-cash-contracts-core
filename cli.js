...
let web3, lightening, circuit, proving_key, groth16, erc20, senderAccount, netId
...
async function deposit({ currency, amount }) {
  ...
  const noteString = `lightening-${currency}-${amount}-${netId}-${note}`
  console.log(`Your note: ${noteString}`)
  if (currency === 'eth') {
    await printETHBalance({ address: lightening._address, name: 'Lightening' })
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
    const value = isLocalRPC ? ETH_AMOUNT : fromDecimals({ amount, decimals: 18 })
    console.log('Submitting deposit transaction')
    await lightening.methods.deposit(toHex(deposit.commitment)).send({ value, from: senderAccount, gas: 2e6 })
    await printETHBalance({ address: lightening._address, name: 'Lightening' })
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
  } else { // a token
    await printERC20Balance({ address: lightening._address, name: 'Lightening' })
    ...
    const allowance = await erc20.methods.allowance(senderAccount, lightening._address).call({ from: senderAccount })
    console.log('Current allowance is', fromWei(allowance))
    if (toBN(allowance).lt(toBN(tokenAmount))) {
      console.log('Approving tokens for deposit')
      await erc20.methods.approve(lightening._address, tokenAmount).send({ from: senderAccount, gas: 1e6 })
    }

    console.log('Submitting deposit transaction')
    await lightening.methods.deposit(toHex(deposit.commitment)).send({ from: senderAccount, gas: 2e6 })
    await printERC20Balance({ address: lightening._address, name: 'Lightening' })
    await printERC20Balance({ address: senderAccount, name: 'Sender account' })
  }
 ...
 * Download deposit events from the lightening, reconstructs merkle tree, finds our deposit leaf
 * in it and generates merkle proof
 * @param deposit Deposit object
 */
async function generateMerkleProof(deposit) {
  // Get all deposit events from smart contract and assemble merkle tree from them
  console.log('Getting current state from lightening contract')
  const events = await lightening.getPastEvents('Deposit', { fromBlock: 0, toBlock: 'latest' })
  const leaves = events
  ...
  const isValidRoot = await lightening.methods.isKnownRoot(toHex(root)).call()
  const isSpent = await lightening.methods.isSpent(toHex(deposit.nullifierHash)).call()
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    try {
      const relay = await axios.post(relayerURL + '/relay', { contract: lightening._address, proof, args })
      if (netId === 1 || netId === 42) {
    ...
    await lightening.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
...
async function loadDepositData({ deposit }) {
  try {
    const eventWhenHappened = await lightening.getPastEvents('Deposit', {
      filter: {
    ...
    const isSpent = await lightening.methods.isSpent(deposit.nullifierHex).call()
...
async function loadWithdrawalData({ amount, currency, deposit }) {
  try {
    const events = await await lightening.getPastEvents('Withdrawal', {
      fromBlock: 0,
      toBlock: 'latest'
    })
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  let contractJson, erc20ContractJson, erc20lighteningJson, lighteningAddress, tokenAddress
  ...
  if (inBrowser) {
    ...
    contractJson = require('./build/contracts/ETHLightening.json')
    circuit = require('./build/circuits/withdraw.json')
    proving_key = fs.readFileSync('build/circuits/withdraw_proving_key.bin').buffer
    ...
    erc20lighteningJson = require('./build/contracts/ERC20Lightening.json')
  }
  ...
  if (isLocalRPC) {
    lighteningAddress = currency === 'eth' ? contractJson.networks[netId].address : erc20lighteningJson.networks[netId].address
    tokenAddress = currency !== 'eth' ? erc20ContractJson.networks[netId].address : null
    senderAccount = (await web3.eth.getAccounts())[0]
  } else {
    try {
      lighteningAddress = config.deployments[`netId${netId}`][currency].instanceAddress[amount]
      if (!lighteningAddress) {
        throw new Error()
      }
      tokenAddress = config.deployments[`netId${netId}`][currency].tokenAddress
    } catch (e) {
      console.error('There is no such lightening instance, check the currency and amount you provide')
      process.exit(1)
    }
  }
  lightening = new web3.eth.Contract(contractJson.abi, lighteningAddress)
...
async function main() {
  if (inBrowser) {
    ...
    program
      ...
      .description('Submit a deposit of specified currency and amount from default eth account and return the resulting note. The currency is one of (ETH|DAI|cDAI|USDC|cUSDC|USDT). The amount depends on currency, see config.js file or visit https://lightening.cash.')
